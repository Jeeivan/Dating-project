import { Link } from "react-router-dom";
import { useState, useEffect } from "react";
import { useParams } from 'react-router-dom';
import { updateComptability } from "../../utlities/updateCompatability";
import { FormData } from "../../utlities/FormData";

export default function Complete() {
    const { id } = useParams();
    const [totalPoints, setTotalPoints] = useState(0)
    const [results, setResults] = useState<FormData | null>(null)
    const [bool, setBool] = useState(false)

    async function fetchFormData() {
    try {
        const response = await fetch(`http://localhost:3006/form/display/single/${id}`)
        const data = await response.json()

        if (response.ok) {
            let sumPoints: number = 0
            const pointsArray: number[] = data.points
            for (let i = 0; i < pointsArray.length; i++) {
                sumPoints += pointsArray[i]
            }
            setTotalPoints(sumPoints)
            setResults(data)
        } else {
            console.log('Failed to fetch form data');
        }
    } catch (error) {
        console.error("Error fetching form data", error)
    }
}


useEffect(() => {
    fetchFormData()
    // eslint-disable-next-line
}, [id])

useEffect(() => {
    updateComptability(id, bool);
}, [bool, id]);

useEffect(() => {
    setBool(totalPoints > 21);
}, [totalPoints]);


console.log(bool);


  return (
    <div>
        <h3>Congrats on finishing the questionnaire!</h3>
        <h2>You achieved a comptability score of: {totalPoints}</h2>
        <Link to="/">
            <button className="all-btns">Start Over</button>
        </Link>
        <br />
        <br />
        {results && results.answers && results.points && (
            <>
         <h4>Your results:</h4>
         <br />
        <div>Name: {results.name}</div>
        <br />
        <div>Q1- Is he a felon- {results.answers[0]} - {results.points[0]} points</div>
        <div>Q2- Your Age: {results.answers[1]}</div>
        <div>Q3- His Age: {results.answers[2]}</div>
        <div>Age Points- {results.points[2]}</div>
        <div>Q4- Height- {results.answers[3]} - {results.points[3]} points</div>
        <div>Q5- His Job- {results.answers[4]} - {results.points[4]} points</div>
        <div>Q6- Her job- {results.answers[5]} - {results.points[5]} points</div>
        <div>Q7- Lines- {results.answers[6]} - {results.points[6]} points</div>
        <div>Q8- Commented something specific- {results.answers[7]} - {results.points[7]} points</div>
        <div>Q9- How far- {results.answers[8]} - {results.points[8]} points</div>
            </>
        )}
    </div>
  )
}
